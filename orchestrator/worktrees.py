...
Owns the per-spec worktree layout helpers, the worktree creation /
restoration / cleanup flows, and the workflow-aware helpers
(`_run_verify_commands`, `_squash_and_force_push`,
`_refresh_base_and_worktrees`, `_sync_worktree_with_base`). Stage
handlers live under `orchestrator/stages/` (decomposition.py,
implementing.py, documenting.py, validating.py, in_review.py,
fixing.py, conflicts.py, question.py) and drive the state machine;
the compatibility facade in `workflow.py` re-exports each helper below
under its original name for backward compatibility with direct test
