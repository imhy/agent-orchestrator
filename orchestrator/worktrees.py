...
Owns the workflow-aware helpers (`_run_verify_commands`,
`_squash_and_force_push`, `_refresh_base_and_worktrees`,
`_sync_worktree_with_base`) plus the conventional-commit /
branch-state probes (`_first_commit_subject`,
`_is_conventional_subject`, `_pr_title_from_commit_or_issue`,
`_head_sha`, `_worktree_dirty_files`, `_branch_ahead_behind`,
...
`_has_new_commits` -- live in `worktree_lifecycle.py`. Both sets of
