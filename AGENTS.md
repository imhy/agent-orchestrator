...
- `orchestrator/` — Python package: tick loop and entry point, label dispatcher / facade (`workflow.py`), per-stage handlers (`stages/`), git and worktree plumbing (`worktrees.py`), hardened git subprocess layer (`git_plumbing.py`, re-exported from `worktrees.py`), process-local issue work scheduler (`scheduler.py`), drift detection (`workflow_drift.py`), prompt builders and parsers (`workflow_messages.py`), agent subprocess runner (`agents.py`), GitHub client (`github.py`), config (`config.py`). Full module-by-module map: [`docs/architecture.md`](docs/architecture.md#top-level-layout).
