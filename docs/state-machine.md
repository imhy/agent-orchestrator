...
`ALLOWED_TRANSITIONS` is composed from a **strict forward spine** (e.g. `implementing → validating → documenting`) plus **interrupt / detour edges modeled per-target** — each from the *exact* set of states that actually emit it (`_INTERRUPT_SOURCES`), not a blanket "any non-terminal". `→ done` and `→ rejected` are allowed only from the PR-having stages where an external merge or close can land — `implementing`, `validating`, `documenting`, `in_review`, `fixing`, `resolving_conflict` (this also covers merged-child recovery via `_finalize_if_pr_merged`, since a finalizable child always carries one of those PR-having labels) — and `→ resolving_conflict` only from the base-sync detour sources (`validating`, `documenting`, `in_review`, `fixing`). `umbrella` and `question` reach `done` via their own forward edge; the pre-PR states (`decomposing`, `ready`, `blocked`) are never terminalized. The entry pseudo-state (unlabeled pickup) only reaches `decomposing` / `implementing`; `question` is operator-applied, has no inbound edge, and finalizes only to `done` (never `rejected`); `done` / `rejected` are terminal. Operator relabels via the GitHub UI bypass both guards (they do not go through `set_workflow_label`), so the guard never fights a human.
...
Each tick fans out across every configured repo (`config.default_repo_specs()` returns one `RepoSpec` per `REPOS` line) and dispatches per-issue handlers through a long-lived `IssueScheduler` capped by `MAX_PARALLEL_ISSUES_GLOBAL` / `MAX_PARALLEL_ISSUES_PER_REPO`. See [`architecture.md#per-tick-flow-workflowtick`](architecture.md#per-tick-flow-workflowtick) for the multi-repo dispatch, `ThreadPoolExecutor`, scheduler lifecycle, and `_run_tick` shutdown / reap details — the rest of this section focuses on the state-machine-relevant parts: label classification, the per-tick base-sync flow (clean rebase + push, with the `resolving_conflict` detour reserved for actual rebase conflicts), the closed-issue sweep, and the pinned-state JSON schema.
...
- **Family-aware labels** — `decomposing`, `blocked`, `umbrella`, and unlabeled (pickup) issues — read and write cross-issue state (parent ↔ child). Two of these running at once could race a parent's child-state write against the child's own handler on a sibling thread. The dispatch loop folds every family-aware issue this tick into ONE bucket submit per repo that drains its issues sequentially on a single worker thread, so the umbrella parent always gets its turn within the same tick (a stale `blocked` child cannot take the slot and starve the parent that would relabel it to `ready`).
  ...
  When every family-aware issue this tick runs a no-agent handler — label in `_CAP_EXEMPT_FAMILY_LABELS` (`blocked` or `umbrella`, both pure label / dep-graph walks) — the bucket submit is marked `cap_exempt=True` and runs on a dedicated executor pool: it does not consume a `MAX_PARALLEL_ISSUES_PER_REPO` / `MAX_PARALLEL_ISSUES_GLOBAL` slot. This keeps a `blocked` parent waiting on its own children from deadlocking those children for the only per-repo slot under the default `parallel_limit=1`. A bucket containing `decomposing` (spawns the decomposer agent) or an unlabeled-pickup issue stays cap-counted. The family mutex still applies, so a follow-up tick that picks up another family issue serializes against the in-flight bucket.
...
- **PR-having worktrees** in `validating` / `documenting` / `in_review` / `fixing` go through `_sync_pr_worktree_to_base`, which attempts the rebase in the refresh itself. On a clean rebase it pushes (force-with-lease pinned to the pre-rebase SHA so a foreign update on the remote rejects the push instead of being clobbered), resets `review_round`, posts a PR notice, and relabels to `validating` so the reviewer re-runs against the rewritten head — the issue does NOT touch `resolving_conflict` for a merely-behind-base case. Only when the rebase actually leaves conflicted files does the helper relabel to `resolving_conflict` (via `_route_pr_worktree_to_resolving_conflict`: seed `conflict_round` only when absent, post a PR notice naming the conflict count, emit `conflict_round` `action="entered"`); the existing `_handle_resolving_conflict` handler then drives the dev agent to resolve the conflict. `documenting` is included so a sibling-PR merge during the brief final-docs hop does not leave the docs commit on a stale base; the handler itself only checks ahead/behind vs. the PR branch.
...
A local-only rebase on a pushed branch would otherwise diverge local HEAD from `pr.head.sha` and break the validating reviewer (it reads local HEAD, so it would review a SHA that isn't on the PR) and `_squash_and_force_push`'s `--force-with-lease=<original_head>` (the lease compares against the un-rebased remote tip). `_sync_pr_worktree_to_base` therefore drives the push in the refresh itself: lease-pinned `--force-with-lease=<branch>:<pre-rebase HEAD>` so a diverged or crash-recovery branch silently rejects the push instead of clobbering work.

Three refresh-only failure modes — push rejection (`auto_base_rebase_push_failed`), rebase failed without conflicted files (`auto_base_rebase_failed`), and dirty-after-clean-rebase (`auto_base_rebase_dirty`) — each reset local HEAD back to the pre-rebase SHA (so the worktree still matches the remote PR head) and park awaiting human with a durable `park_reason`. Parking is required because `tick()` runs the base refresh BEFORE dispatching the same issue's handler: without it the `validating` / `documenting` / `in_review` / `fixing` handler would otherwise run on a behind-base PR head this same tick and (for in_review) could fire the HITL ready-ping on a stale-base PR. Recovery is refresh-only and gated on a new human issue-thread comment — full contract under "Durable pinned `park_reason` values" below.

The flow deliberately does NOT call `_bump_in_review_watermarks` (the `_handle_in_review` analog runs that AFTER scanning new comments — running it here, before any handler scans, would silently mark unread human "do not merge" / fix-request comments as consumed and they would never reach the dev). The orchestrator's own PR notice is filtered out via `orchestrator_comment_ids` on the next in_review scan, so leaving the watermark alone is safe. The `pending_fix_*` bookmarks recorded by an `in_review` → `fixing` route are similarly left untouched: the next handler that resumes that route still finds them, and a stale bookmark on a now-`validating` issue is harmless (the reviewer pass clears it naturally when it next bounces to `fixing`).

The flow also skips when `awaiting_human=True` with a `park_reason` not in `_AUTO_REBASE_PARK_REASONS` because attempting an auto-rebase would either re-open work the human is meant to resolve or undermine the `MAX_REVIEW_ROUNDS` / `MAX_CONFLICT_ROUNDS` caps that exist precisely to require human intervention after repeated failures. Auto-rebase parks (`auto_base_rebase_failed` / `auto_base_rebase_dirty` / `auto_base_rebase_push_failed`) are the exception: a new human issue comment past `last_action_comment_id` is captured as `unparking_consumed_max` (the local retry intent) but the actual `awaiting_human` / `park_reason` clear + watermark advance is DEFERRED to the same pinned-state write that commits real progress (recovery's case-2/3 success / fall-through state writes, or the normal-flow anchor set just before `_rebase_base_into_worktree`). Every downstream gate that early-returns without making progress (`hold_base_sync`, `gh.get_pr()` failure, terminal PR, case-1 fall-through + dirty / `behind == 0`) therefore leaves the on-disk park UNTOUCHED — the operator's comment stays ahead of the watermark so the next refresh tick rediscovers it, and the same-tick stage-handler dispatch still sees `awaiting_human=True` + an auto-rebase `park_reason` and stays silent (see the handler short-circuit below). A subsequent rebase / push failure inside the recovery or normal flow re-parks via `_park_auto_rebase_failure`, which ratchets `last_action_comment_id` to the live latest so a quick second human comment is not silently consumed by the re-park.

Each PR-stage handler (`_handle_in_review` / `_handle_validating` / `_handle_fixing` / `_handle_documenting`) short-circuits at its `awaiting_human` gate when `park_reason` is in `_AUTO_REBASE_PARK_REASONS` -- regardless of whether new comments are present. The refresh OWNS the operator's retry comment; routing it through the in_review → fixing path, resuming the dev session, respawning the reviewer, or running a docs pass would consume the comment as input for the wrong stage and silently drop the retry intent. The check is gated on the park reason, not the comment presence, so the refresh's deferred-clear semantics interlock cleanly with the dispatcher.

Before rebasing, the flow fetches `gh.get_pr(pr_number)` and skips when `pr_state != "open"`: a just-merged PR advances `<remote>/<base>`, so the still-validating / still-documenting / still-in_review / still-fixing worktree pointed at the now-stale branch is naturally behind base; without this gate the refresh would push, post an "auto-rebased" notice, and relabel to `validating` on a PR the next handler call would finalize to `done` (or `rejected` for a closed-without-merge PR).
...
Rebase is used across both paths to keep issue branches linear after sibling PRs land. Dirty worktrees (in-flight agent edits, crash-recovered trees) are skipped both before the rebase and again before the push, so a post-rebase uncommitted edit cannot be force-pushed alongside the rebase result. On a pre-PR content conflict the rebase is aborted so the worktree stays on its pre-rebase SHA. For PR branches, every pushed rebase resets `review_round`, so the reviewer must approve the rewritten head before the in_review ready-ping gate can pass. Failures are logged and swallowed; keeping every issue moving matters more than perfect base sync.
...
- `awaiting_human`, `last_action_comment_id`, `park_reason` — generic HITL park triple. `_park_awaiting_human` always sets `awaiting_human=True`, ratchets `last_action_comment_id`, and **clears pinned `park_reason` to `None`** — its own `reason=` argument is recorded only in the emitted `park_awaiting_human` audit event. A handler that needs `park_reason` to survive into the next tick (so the awaiting-human resume can gate on the cause) explicitly re-sets it via `state.set("park_reason", "<tag>")` after the park call; everything else leaves the field cleared and the typed tag lives only in the audit event.
  ...
  **Durable pinned `park_reason` values** (re-set after `_park_awaiting_human`, so subsequent ticks read them):
  ...
  - `_sync_pr_worktree_to_base` (the per-tick base-sync flow, not a stage handler) — `auto_base_rebase_failed` (rebase failed for a non-conflict reason; the worktree was restored to the pre-rebase SHA via `git rebase --abort`), `auto_base_rebase_dirty` (clean rebase landed but left an uncommitted edit; HEAD was reset back and untracked files cleaned), `auto_base_rebase_push_failed` (clean rebase landed but `--force-with-lease` rejected the push; HEAD was reset back to the pre-rebase SHA). All three are persisted by the shared `_park_auto_rebase_failure` helper. **Recovery is refresh-only and gated on a new human issue-thread comment**, and the clear is **deferred** to the same pinned-state write that publishes real progress so a downstream early-return path cannot silently drop the retry intent — no stage handler reconciles these reasons, and future `_sync_pr_worktree_to_base` runs skip awaiting-human issues by default. When the parked issue gets a new comment past `last_action_comment_id`, the refresh's awaiting-human gate captures the watermark intent as `unparking_consumed_max` but DOES NOT immediately clear `awaiting_human` / `park_reason` or write pinned state. The actual clear + watermark advance lands on disk only when the recovery's case-2/3 success / fall-through state write commits or when the normal-flow anchor set `state.set("pending_auto_base_rebase_push_sha", before_sha)` writes; downstream early-return gates (`hold_base_sync`, `gh.get_pr()` failure, terminal PR, case-1 fall-through + dirty / `behind == 0`) leave the on-disk park intact so the operator's comment is rediscovered next tick. A subsequent failure during the recovery or normal flow re-parks (with `_park_awaiting_human` re-ratcheting `last_action_comment_id` to the live latest so a quick follow-up comment is not silently consumed). The dispatcher's same-tick `_handle_in_review` / `_handle_fixing` / `_handle_validating` / `_handle_documenting` short-circuits whenever `park_reason in _AUTO_REBASE_PARK_REASONS` (regardless of new-comment presence) so the operator's "retry the rebase" reply does NOT get consumed as in_review feedback / a dev-resume trigger / a docs-pass trigger / a reviewer respawn signal -- the refresh owns the comment.
...
- `pending_auto_base_rebase_push_sha` — crash-recovery anchor for `_sync_pr_worktree_to_base`. Set to the pre-rebase local HEAD (and pinned to GitHub) immediately BEFORE `_rebase_base_into_worktree` is called; cleared on every exit (success, conflict route, dirty-after-rebase park, push-failure park, unreadable-pre-rebase-HEAD park, unreadable-post-rebase-HEAD park, no-op rebase, label-no-longer-in-refresh-set bailout, terminal-PR bailout). `hold_base_sync` deliberately PRESERVES the anchor so removing the hold later runs recovery against the original pre-rebase SHA. A non-empty value on entry means a previous tick rebased and then died before the post-push relabel / `review_round=0` write — `_recover_pending_auto_base_rebase` keys off it and routes one of: (1) HEAD == anchor → no-op rebase, clear flag and fall through to the normal-flow's dirty + behind checks; (2) HEAD == remote PR head (push landed on prior tick) → emit `base_rebased(method="crash_recovery_relabel_only")` and reset `review_round=0` + clear anchor; (3) HEAD ahead of remote PR head (push pending) → dirty-check + reset+`git clean -fd`+park on dirty, otherwise push with `--force-with-lease=<anchor>` and emit `base_rebased(method="crash_recovery_pushed")` + reset `review_round=0` + clear anchor; (4) HEAD diverged from remote PR head → reset HEAD to anchor and park with `auto_base_rebase_push_failed`. Cases 2 and 3 then `behind`-split: when this tick's `behind == 0` against the freshly-fetched base the recovery flips the label straight to `validating` and returns True; when `behind > 0` (base advanced again since the interrupted rebase) the recovery WRITES the state changes (cleared anchor, `review_round=0`, the recovery PR notice + audit event) but DOES NOT relabel, returning False so the caller's normal rebase + push flow rebases the recovered head onto the newer base and routes to `validating` itself — the audit trail then carries TWO `base_rebased` events (`crash_recovery_*` then `auto_clean_rebase`). Cannot-verify exits (fetch failure, rev-parse failure, empty remote SHA, `_branch_ahead_behind` `(0, 0)` with a SHA mismatch) reset HEAD to the anchor and park with `auto_base_rebase_push_failed` so same-tick stage handlers cannot run against an unverified local SHA. The outer `_sync_worktree_with_base` dirty hard-skip therefore applies ONLY to pre-PR worktrees; PR-having worktrees always route into `_sync_pr_worktree_to_base` so case 3 above can reset+clean+park a crash-mid-rebase dirty tree instead of being silently skipped. Without this anchor, the next refresh tick's `HEAD..<remote>/<base>` check would report `behind == 0` (because the local rebase already consumed base) and skip the worktree, letting validating review a SHA not on the PR or in_review / documenting / fixing run on stale label + `review_round` state.
...
- **Trigger**: each tick while label is `resolving_conflict` (set by an operator relabel or by the per-tick base-sync flow in `_refresh_base_and_worktrees` when the auto rebase actually left conflicted files — a merely-behind-base PR rebase + push lands directly on `validating` and never enters this stage). Also runs on closed-`resolving_conflict` issues yielded by the closed-issue sweep, mirroring the in_review terminal handling so a manually-merged PR finalizes to `done` even when `Resolves #N` already closed the issue.
...
```
   ...
   (the orchestrator never merges from in_review -- humans drive the
    merge; the merged arc below always reflects an external merge):
     ...
     resolving_conflict (operator relabel or per-tick base-sync flow
       when the auto rebase actually left conflicted files):
