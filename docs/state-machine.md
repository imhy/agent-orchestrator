...
- `docs_final_pending` — boolean marker set by `_handle_validating`'s approval branch right before flipping the label to `documenting`. `_handle_documenting` clears it on every success exit alongside the route to `in_review`, and the drift block clears it (with the rest of the approval bookkeeping) when a body edit invalidates the prior approval mid-hop. **`_handle_validating` also clears it at the top of every tick** so any prior cycle's stale marker that survived an aborted handoff (operator relabel back from documenting, drift-induced relabel back to validating, PR-worktree refresh detour landing on validating) cannot be carried by a later CHANGES_REQUESTED / awaiting-human / drift / transient-park exit into a future documenting round.
...
- **Internal flow**:
  ...
  8. Branch on the post-agent state. Every success exit calls one of two helpers (`_advance_after_docs_push` / `_advance_after_docs_no_change`) that route to **`in_review`** (clear `docs_final_pending` AND its companion sentinel `final_docs_approval_seeded`; for the push branch update `agent_approved_sha` to the new head **only when `final_docs_approval_seeded` is set** — when validating's `gh.get_pr()` raised the seeding block never ran, so any non-`None` `agent_approved_sha` left in state is a stale snapshot from a prior round and must NOT be promoted to the new docs head, leaving AUTO_MERGE gated; and ratchet `pr_last_comment_id` via `_ratchet_in_review_watermark_for_final_docs` past any issue-thread reply the awaiting-human resume consumed so the next in_review tick does not bounce the issue to `fixing` over already-addressed feedback). Branches:
...
```
   ...
   resolving_conflict (AUTO_MERGE only, capped by MAX_CONFLICT_ROUNDS):
     ...
     (docs do not run here -- the single docs pass runs after the
      reviewer's final approval via docs_final_pending)
