...
| `fixing` | Unread in-review feedback (any of the four PR comment surfaces) is queued during the quiet window or actively being addressed by the dev fix-loop. A successful fix — and a rescan finding no unread feedback — both bounce DIRECTLY back to `validating` so the reviewer re-approves before the in_review ready-ping gate can pass. Docs are deferred to the final-docs handoff after reviewer approval. |
