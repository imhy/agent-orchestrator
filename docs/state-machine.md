...
A single long-lived `IssueScheduler` (see `orchestrator/scheduler.py`) is built once at startup with `global_cap=MAX_PARALLEL_ISSUES_GLOBAL` (default 3) and `per_repo_cap=MAX_PARALLEL_ISSUES_PER_REPO` (default 1), and threaded through every `workflow.tick(gh, spec, scheduler=...)` call. The scheduler owns the in-flight set, the per-repo counters, the family-aware mutex, and the `ThreadPoolExecutor` that actually runs the per-issue handlers. Each tick enumerates its pollable issues and submits one callable per issue — the tick itself returns as soon as the submissions are made, without waiting for handler completion. Total cap-counted in-flight handlers across all repos never exceeds `MAX_PARALLEL_ISSUES_GLOBAL` regardless of how many `parallel_limit` slots each repo declares. Cap-exempt no-agent buckets (`blocked` / `umbrella` only — see the family-aware classification below) are excluded from this count by design and run on a dedicated executor independent of `global_cap`.

The process-local in-flight set is keyed on `(repo_slug, issue_number)`: an issue running in one repo does not block the same issue number in another repo. The duplicate-active gate consults this set on every `submit`, so a slow handler that straddles polling passes does NOT race a fresh submit from the next polling pass for the same key — the second submit is rejected with `reason=duplicate_active` (logged at DEBUG so a busy repo does not spam the operator log) and the next poll re-enumerates. The marker is cleared in the worker's done-callback under the scheduler's reentrant lock, so the very next polling pass after a worker exits is free to re-dispatch the same key; the `track_active` helper carries a separate set (`_tracked`) used by the family-bucket drain so per-iteration claims for the issue currently being processed inside the bucket report True from `is_active` without inflating the global / per-repo cap counters.

`main` shuts the scheduler down (`wait=True`) on every exit path (`--once` returning, SIGINT/SIGTERM signal handler firing, self-modifying-merge restart) so in-flight workers complete cleanly. The signal handler (`main._shutdown`) additionally calls `scheduler.shutdown(wait=False)` synchronously the moment the signal lands, so the scheduler's submit path is CLOSED before the in-progress tick returns. Without that, an iterating `workflow.tick` would keep landing fresh `scheduler.submit` calls for the rest of its dispatch loop after the user already asked to stop -- the `_running=False` flag alone only guards the next tick boundary, not the submit loop already underway. With the early shutdown, the iterating dispatch sees `reason=closed` on every remaining submit and the finally-block `shutdown(wait=True)` waits only on workers that were already accepted, not on freshly-enqueued post-signal work. `shutdown` closes the submit path first (so a tick currently iterating cannot keep enqueueing work after shutdown was requested) and then waits for in-flight workers to exit, and finally drains any failure-completion records through `reap` so a worker that raised on the way out still surfaces in the orchestrator log. Repeated calls are safe: a prior `shutdown(wait=False)` does not short-circuit a follow-up `shutdown(wait=True)`, and the trailing reap on the second call catches any completion that landed in between.

`_run_tick` itself calls `scheduler.reap()` exactly once per polling pass (right before `analytics.prune_with_retention_logging`), so any worker that finished since the last poll has its failure drained before the next polling iteration begins -- the contract is one reap per polling pass regardless of repo count, and `_dispatch_via_scheduler` deliberately does NOT reap.
...
- **Family-aware labels** — `decomposing`, `blocked`, `umbrella`, and unlabeled (pickup) issues — read and write cross-issue state (parent ↔ child). Two of these running at once could race a parent's child-state write against the child's own handler on a sibling thread. The dispatch loop folds every family-aware issue this tick into ONE bucket submit per repo (`family=True`, sentinel issue number `_FAMILY_BUCKET_ISSUE = 0`) that drains its issues sequentially on a single worker thread. Per-family-issue submits would let a stale child (e.g. a no-op `blocked` waiting on a dependency that's already done) take the family slot and starve the parent umbrella issue whose handler is what would actually relabel the child to `ready`; the bucket guarantees the umbrella always gets its turn within the same tick. Each per-issue iteration in the bucket wraps `_process_issue` in `scheduler.track_active(spec.slug, n)` so `is_active(repo, n)` keeps reporting True for the family issue currently being processed — without that the refresh-skip contract would observe only the bucket's sentinel key and race the agent on the actual issue. When every family-aware issue this tick runs a no-agent handler — label in `_CAP_EXEMPT_FAMILY_LABELS` (`blocked` or `umbrella`, both pure label/dep-graph walks) — the bucket submit is marked `cap_exempt=True` and runs on a dedicated executor pool: such a bucket must always get its turn, so it does not consume a `MAX_PARALLEL_ISSUES_PER_REPO` / `MAX_PARALLEL_ISSUES_GLOBAL` slot. This is what keeps a `blocked` parent waiting on its own children from deadlocking those children for the only per-repo slot under the default `parallel_limit=1`. A bucket containing `decomposing` (spawns the decomposer agent) or an unlabeled-pickup issue stays cap-counted. The family mutex still applies, so a follow-up tick that picks up another family issue still serializes against the in-flight bucket.
...
When the scheduler rejects a submit (duplicate active issue, cap reached, family slot held), the call is logged with the precise reason — `duplicate_active` at DEBUG (common while a long-running worker straddles ticks), the rarer `family_slot_held` / `global_cap` / `per_repo_cap` / `closed` at INFO — and the issue is simply skipped this tick. The next polling pass re-enumerates and re-submits against the live scheduler state. The "active issues are skipped until completion" guarantee extends to the pre-tick base refresh: `_refresh_base_and_worktrees` consults `scheduler.is_active(spec.slug, issue_number)` per worktree and skips any active issue's worktree so a base advance cannot rebase a pre-PR worktree under a running agent or relabel a PR-having worktree mid-handler.
