...
- **Output**: label moved to `done` / `rejected` (terminal, external merge / close) OR a relabel to `fixing` (fresh PR feedback) OR a HITL park (unmergeable / missing pr_number) OR a HITL ping (approved + mergeable PR, no relabel) OR a no-op tick.
