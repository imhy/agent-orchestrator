...
- **Output**: terminal `done` / `rejected`, OR label flipped to `validating` (pushed fix OR no-new-feedback bounce), OR label flipped to `resolving_conflict` (stuck validating-route transient park while the worktree is out of sync with the PR — behind base or an unpushed local rebase), OR label flipped to `in_review` (in_review route, explicit nothing-to-fix ACK fast path), OR a HITL park, OR a no-op (quiet-window wait, missing-PR park already set).
