...
| `documenting` | The dev session runs the single docs pass on the existing PR worktree. Reached only via the **final-docs handoff**: after `_handle_validating` approves + verifies + squashes, the label flips to `documenting` and (when validating actually persisted `agent_approved_sha` this round) the `final_docs_approval_seeded` sentinel is set. The docs pass advances to `in_review` (updating `agent_approved_sha` to the new pushed head when a docs commit lands AND `final_docs_approval_seeded` is set, so the in_review ready-ping gate `agent_approved_sha == pr.head.sha` survives; if validating's PR snapshot failed and left the sentinel absent, the docs push leaves `agent_approved_sha` untouched so the ready-ping gate stays closed). An explicit `DOCS: NO_CHANGE` verdict against a remote-clean head advances without pushing. There is no pre-approval entry: every `implementing` PR open, every pushed fix in `validating` / `fixing`, every `in_review` drift push, and every `resolving_conflict` pushed exit hands straight back to `validating` so the reviewer re-runs against the new branch directly. |
| `validating` | The reviewer agent is checking the diff and may bounce fixes back to the dev agent. |
| `in_review` | A PR is open and ready for human review and manual merge. The orchestrator never merges from here -- humans drive the merge. An approved + mergeable PR (no standing human CHANGES_REQUESTED on the current head) earns a one-shot HITL ping per head SHA; an unmergeable PR parks awaiting human attention. |
...
A local-only rebase on a pushed branch would otherwise diverge local HEAD from `pr.head.sha` and break the validating reviewer (it reads local HEAD, so it would snapshot `agent_approved_sha` to a SHA that isn't on the PR) and `_squash_and_force_push`'s `--force-with-lease=<original_head>` (the lease compares against the un-rebased remote tip). `_handle_resolving_conflict` just does rebase + push + relabel.
...
- `agent_approved_sha` — the head SHA the reviewer agent OK'd. `_handle_in_review` keys its HITL ready-ping on this (the reviewer agent posts an issue comment, not a real PR review, so `pr_is_approved` alone would never be True for the agent flow); the ping only fires when `agent_approved_sha == pr.head.sha` OR a human/bot submitted a real APPROVED review on the current head, so the orchestrator never advertises an unreviewed commit as ready for human merge. Seeded by `_handle_validating`'s approval branch (post-squash local head, or the reviewed head when squash is off) and **then updated by `_handle_documenting` on the final-docs hop** when a docs commit lands AND the slot was already seeded — the docs pass owns the `agent_approved_sha == pr.head.sha` invariant across the new head. When validating's PR snapshot raises (recoverable network/PyGithub blip), it deliberately leaves the slot unset; the docs push respects that and does NOT seed it, so the ready-ping gate stays closed until the next reviewer round approves explicitly. Cleared by `_handle_validating` on every pushed dev fix (CHANGES_REQUESTED, awaiting-human resume, user-content drift "pushed", transient-park recovery push) so the freshly-pushed head cannot inherit a stale prior approval, by `_handle_fixing` whenever it pushes a fix (which then bounces directly back to `validating`), by `_handle_in_review`'s drift pushed and ACK outcomes (both bounce directly back to `validating`), and by `_handle_documenting`'s drift block when a body edit fires during the final-docs hop (relabels to `validating` and resets `review_round=0` so the reviewer re-evaluates the updated body on the next tick).
- `final_docs_approval_seeded` — sentinel set by `_handle_validating`'s approval branch in the SAME code block that seeds `agent_approved_sha` and the in_review watermarks (inside the `else` arm of its `gh.get_pr()` try) **and only when the local SHA being persisted is non-empty**: if `_head_sha()` returned empty earlier in the tick, `agent_approved_sha` is never actually persisted this round and the sentinel must stay False so the docs push cannot promote the docs commit's SHA into a slot the reviewer never confirmed. `_handle_documenting`'s push exit gates the `agent_approved_sha` update on this sentinel — without it (validating's PR snapshot raised OR its local SHA was empty) a stale non-`None` `agent_approved_sha` left over from an earlier round would be silently promoted to the new docs head and the in_review ready-ping gate could pass against a SHA THIS round's reviewer never confirmed. The `documenting` label itself is the handoff signal; the sentinel only gates the docs-push SHA promotion. Documenting clears the sentinel on every success exit and in the drift block, and `_handle_validating` clears it at the top of every tick so an aborted prior approval handoff (operator relabel back from documenting, drift-induced relabel, PR-worktree refresh detour landing on validating) cannot be carried by a later CHANGES_REQUESTED / awaiting-human / drift / transient-park exit into a future documenting round.
...
`_detect_user_content_change` durably persists the baseline on its FIRST encounter via `gh.write_pinned_state` so an early-return tick (awaiting-human-with-no-new-comments, child-waiting-on-deps, debounce) cannot silently absorb a later edit as the new baseline. On drift the action depends on where in the lifecycle the issue is:
  ...
  - `implementing` / `validating` / `in_review` / `resolving_conflict` (a dev session exists and possibly a PR) → on drift the handler:
    ...
    - For the `in_review` drift specifically, BOTH the "pushed" and "ack" outcomes reset `review_round` and clear `agent_approved_sha` -- a content drift invalidates the prior reviewer approval (it was for the old requirements), so the in_review ready-ping gate must not advertise the PR as ready until the reviewer re-evaluates against the updated body/comments. Both outcomes also share the same destination: a DIRECT bounce back to `validating`. Docs do not run here -- the single docs pass is deferred to the final-docs handoff after reviewer approval.
...
- **Internal flow**:
  ...
  8. Branch on the post-agent state. Every success exit calls one of two helpers (`_advance_after_docs_push` / `_advance_after_docs_no_change`) that route to **`in_review`** (clear the `final_docs_approval_seeded` sentinel; for the push branch update `agent_approved_sha` to the new head **only when `final_docs_approval_seeded` was set** — when validating's `gh.get_pr()` raised the seeding block never ran, so any non-`None` `agent_approved_sha` left in state is a stale snapshot from a prior round and must NOT be promoted to the new docs head, leaving the in_review ready-ping gate closed; and ratchet `pr_last_comment_id` via `_ratchet_in_review_watermark_for_final_docs` past any issue-thread reply the awaiting-human resume consumed so the next in_review tick does not bounce the issue to `fixing` over already-addressed feedback). Branches:
     ...
     - no commit + `DOCS: NO_CHANGE` verdict:
       - if `ahead > 0` (a prior tick committed but never landed the push), push the recovered commit and advance via `_advance_after_docs_push(after_sha)` — the in_review ready-ping gate must key on the new head, so the local-only commit can't be left behind. A push failure parks with `push_failed`.
       - otherwise persist `docs_checked_sha=after_sha`, `docs_verdict="no_change"`, reset `silent_park_count=0`, post `:books: documenting pass: no docs changes required.` (with the dev's justification quoted when present), then `_advance_after_docs_no_change()` — no commit landed, so the PR head still matches `agent_approved_sha` and the approval invariant survives the docs hop untouched.
...
- **Output**: label moved to `in_review` (pushed docs commit updates `agent_approved_sha` and advances; no-change verdict advances with `agent_approved_sha` unchanged) OR label moved to `validating` (drift unwind: body edit invalidated the prior approval, no docs spawn this tick) OR terminal `done`/`rejected` (external-merge / closed-issue short-circuit) OR a HITL park (`missing_pr_number`, `fetch_failed`, `diverged_branch`, `worktree_reset_failed`, `agent_timeout`, `push_failed`, `dirty_worktree`, `agent_question`, `agent_silent`).
...
- **Internal flow**:
  ...
  4. Parse last `VERDICT:` marker (`_parse_review_verdict`):
     - `approved` → in this order:
       ...
       5. Snapshot `agent_approved_sha` from the **local SHA the reviewer (or the squash) produced** — explicitly *not* the current remote PR head. If the remote moves out from under us, `agent_approved_sha != pr.head.sha` in the in_review approval gate and the ready-ping waits for a fresh review round.

          With `SQUASH_ON_APPROVAL=off`, the snapshot is the pre-review local HEAD (`reviewed_sha` captured before `run_agent`).
       6. Seed the in_review comment watermarks (inside the `else` arm of the `gh.get_pr()` try so a snapshot failure leaves them untouched). When the local SHA being persisted is non-empty (`new_head_sha` truthy — the post-squash local head, or the reviewed head when squash is off), set `agent_approved_sha` to that SHA AND set the `final_docs_approval_seeded=True` sentinel in the same `if` block; when the local SHA is empty (`_head_sha()` returned empty earlier), neither is touched so the sentinel stays False. THEN, outside the try, relabel to `documenting`. The `documenting` label itself is the handoff signal; the sentinel only gates the docs-push SHA promotion. `_handle_documenting`'s success exits always advance to `in_review` and, on a pushed docs commit, update `agent_approved_sha` to the newly-pushed head so the in_review ready-ping gate (`agent_approved_sha == pr.head.sha`) survives the final docs commit (gated on `final_docs_approval_seeded` so a stale non-`None` `agent_approved_sha` from a prior round cannot be promoted when this round's snapshot failed or its local SHA was empty); they also ratchet `pr_last_comment_id` past any issue-thread reply consumed by the awaiting-human resume so the in_review tick does not bounce the issue to `fixing` over already-addressed feedback. The approval comment, squash comment, and `agent_approved_sha` snapshot seeded here all ride through the documenting hop untouched.
     - `unknown` (no marker) → park.
     - `changes_requested` → post the feedback to the PR, then **resume the developer's session** on its locked spec (backend + args) with the fix prompt; if it produces a new commit on a clean tree, push, increment `review_round`, and clear `agent_approved_sha` so the next reviewer round re-snapshots the freshly-pushed head. The label stays on `validating` (no relabel emitted) so the reviewer re-evaluates the new head on the next tick. A no-commit reply parks via `_on_question` and stays on `validating`.
- **Output**: label moved to `documenting` (approval after verify + squash, so the docs pass hands off to `in_review`) OR no label change with `agent_approved_sha` cleared (a new fix commit landed via CHANGES_REQUESTED, awaiting-human resume, user-content drift, or a transient-park-recovery push that finished a pending push — the issue stays on `validating` and the reviewer re-evaluates on the next tick) OR a HITL park (squash/force-push failure stays on `validating` with the approval comment already on the PR; every other park branch keeps the existing label).
...
- **Output**: label moved to `done` / `rejected` (terminal, external merge / close) OR a relabel to `fixing` (fresh PR feedback) OR a HITL park (unmergeable / missing pr_number) OR a HITL ping (mergeable PR, no relabel) OR a no-op tick.
...
- **Internal flow**:
  ...
  10. **On a pushed fix**: clear the `pending_fix_*` bookmarks (they served their purpose), reset `review_round` to 0 so the reviewer starts fresh on the new diff, drop the now-stale `agent_approved_sha` (the head just moved — the reviewer must re-approve), write pinned state, and flip the label DIRECTLY back to `validating` so the reviewer re-evaluates the new head next tick. Docs do not run on this exit — the single docs pass is deferred to the final-docs handoff after reviewer approval.
...
```
   ...
   Validating fix loop (any pushed dev fix):
     ...
     validating --(APPROVED, verify ok, squash ok)──►
       seed final_docs_approval_seeded, label=documenting (final-docs)
         ──► docs pass (push updates agent_approved_sha when validating
              seeded it, else leaves None; no_change keeps the existing
              value; the exit also ratchets pr_last_comment_id past any
   ...
   In_review terminals and fix bounce:
     ...
     in_review --(user-content drift, pushed)──► validating
       (review_round=0, agent_approved_sha cleared; docs do not run
        here -- the single docs pass is deferred to the final-docs
     ...
     in_review --(user-content drift, ACK no-commit)──► validating
       (review_round=0, agent_approved_sha cleared; same destination
        as the pushed exit)
     in_review --(approved, mergeable, no human CHANGES_REQUESTED,
