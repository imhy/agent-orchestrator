...
| `documenting` | The dev session runs a docs pass on the existing PR worktree before the reviewer sees the diff: any README / docs / plans edits are committed, pushed to the PR branch, and the issue advances to `validating`. Reached after every code-changing branch update (initial PR open from `implementing`, validating fix, fixing-stage PR-feedback fix, in_review drift push, every `resolving_conflict` push). An explicit `DOCS: NO_CHANGE` verdict against a remote-clean head advances without pushing. |
...
- **Trigger**: each tick while the label is `documenting` (set by `_handle_implementing` after `_on_commits` opens the PR, and by every pushed-fix exit in `_handle_validating` / `_handle_fixing` / `_handle_in_review` drift / `_handle_resolving_conflict`). Also runs on closed-`documenting` issues yielded by the polling sweep so an externally-merged PR finalizes to `done`.
- **Input**: pinned `pr_number`, `branch`, `dev_agent`/`dev_session_id` (the docs pass runs AS the dev role and reuses the same locked spec / session id — there is no separate `documenting_agent`), plus `docs_checked_sha` / `docs_verdict` / `silent_park_count` watermarks.
- **Internal flow**:
  ...
  8. Branch on the post-agent state:
     ...
     - new commit on a clean tree → `_push_branch` (with the same hardened path the implementing push uses). On success record `docs_checked_sha=after_sha`, `docs_verdict="updated"`, reset `silent_park_count=0`, post `:books: documenting pass: pushed docs commit.` (or the recovered-commit variant) to the PR, set label `validating`. A push failure parks with `park_reason="push_failed"`.
     - no commit + `DOCS: NO_CHANGE` verdict:
       - if `ahead > 0` (a prior tick committed but never landed the push), push the recovered commit and advance — the reviewer must see the docs in the diff at validating, so the local-only commit can't be left behind. A push failure parks with `push_failed`.
       - otherwise persist `docs_checked_sha=after_sha`, `docs_verdict="no_change"`, reset `silent_park_count=0`, post `:books: documenting pass: no docs changes required.` (with the dev's justification quoted when present), set label `validating`.
...
- **Output**: label moved to `validating` (pushed docs commit, recovered docs commit pushed under a no-change verdict, or a true no-change verdict against the remote-clean head) OR terminal `done`/`rejected` (external-merge / closed-issue short-circuit) OR a HITL park (`missing_pr_number`, `fetch_failed`, `diverged_branch`, `agent_timeout`, `push_failed`, `dirty_worktree`, `agent_question`, `agent_silent`).
