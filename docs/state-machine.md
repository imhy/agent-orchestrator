...
`_detect_user_content_change` durably persists the baseline on its FIRST encounter via `gh.write_pinned_state` so an early-return tick (awaiting-human-with-no-new-comments, child-waiting-on-deps, debounce) cannot silently absorb a later edit as the new baseline. On drift the action depends on where in the lifecycle the issue is:
  ...
  - `implementing` / `validating` / `in_review` / `resolving_conflict` (a dev session exists and possibly a PR) → on drift the handler:
    ...
    - a clean pushed fix routes to `documenting` from `validating` (with `review_round++`); hands DIRECTLY back to `validating` from `resolving_conflict` and from `in_review` (both pre-approval exits deliberately skip the `documenting` hop -- the single docs pass is deferred to the post-approval hop, so the reviewer re-runs against the new branch directly); or runs the implementing `_on_commits` path to open/push the PR from `implementing`;
...
- **Internal flow**:
  ...
  6. Parse the manifest from `result.last_message` via `_parse_manifest` (regex captures the fenced ` ```orchestrator-manifest ` block; structural validation rejects unknown decisions, bad child shape, self-deps, cycles, and >10 children):
     ...
     - **decision == "split"** → crash-safe creation in three phases. The decomposer is no longer asked to emit a final docs-update child — the post-implementing `documenting` stage and the validating-side fix exits still flow through `documenting` before the reviewer re-runs, while the `fixing` PR-feedback push, the `in_review` drift push, and every `resolving_conflict` push hand straight back to `validating` so the reviewer re-runs against the new branch directly; the single final docs pass after the reviewer's `VERDICT: APPROVED` keeps docs in sync for everything else, so docs stay current without a synthetic child.
...
- **Output**: label moved to `documenting` (approval after verify + squash, with `docs_final_pending=True` so the docs pass hands off to `in_review`; OR a new fix commit landed via CHANGES_REQUESTED, awaiting-human resume, user-content drift, or a transient-park-recovery push that finished a pending push — these set no marker so the docs pass bounces back to `validating`) OR a HITL park (squash/force-push failure stays on `validating` with the approval comment already on the PR; every other park branch keeps the existing label).
...
```
   Forward (single-task happy path):
     (none) ──► decomposing ──► ready ──► implementing ──► documenting
                ──► validating ──► documenting (final-docs, docs_final_pending=True)
   ...
   Validating fix loop (any pushed dev fix):
     validating --(CHANGES_REQUESTED / awaiting-human resume / user-content drift / transient-park push)──►
       ++review_round, label=documenting (no marker) ──► docs pass ──► validating
