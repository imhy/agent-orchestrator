...
| `documenting` | The dev session runs the single docs pass on the existing PR worktree. Reached only via the **final-docs handoff**: after `_handle_validating` approves + verifies + squashes, the label flips to `documenting` and (when validating actually persisted `agent_approved_sha` this round) the `final_docs_approval_seeded` sentinel is set. The docs pass advances to `in_review` (updating `agent_approved_sha` to the new pushed head when a docs commit lands AND `final_docs_approval_seeded` is set, so the AUTO_MERGE `agent_approved_sha == pr.head.sha` invariant survives; if validating's PR snapshot failed and left the sentinel absent, the docs push leaves `agent_approved_sha` untouched so AUTO_MERGE stays gated). An explicit `DOCS: NO_CHANGE` verdict against a remote-clean head advances without pushing. There is no pre-approval entry: every `implementing` PR open, every pushed fix in `validating` / `fixing`, every `in_review` drift push, and every `resolving_conflict` pushed exit hands straight back to `validating` so the reviewer re-runs against the new branch directly. |
...
| `fixing` | Unread in-review feedback (issue thread / PR conversation / inline review / PR review summary) or a human CI-fix request is queued during the quiet window or actively being addressed by the dev fix-loop. A successful fix flips DIRECTLY back to `validating` so the reviewer re-approves before auto-merge can proceed; when the rescan finds no unread feedback at all the bounce also targets `validating`. Docs are not run here -- they land once in the final-docs pass after reviewer approval. |
...
`_detect_user_content_change` durably persists the baseline on its FIRST encounter via `gh.write_pinned_state` so an early-return tick (awaiting-human-with-no-new-comments, child-waiting-on-deps, debounce) cannot silently absorb a later edit as the new baseline. On drift the action depends on where in the lifecycle the issue is:
  ...
  - `implementing` / `validating` / `in_review` / `resolving_conflict` (a dev session exists and possibly a PR) → on drift the handler:
    ...
    - For the `in_review` drift specifically, BOTH the "pushed" and "ack" outcomes reset `review_round` and clear `agent_approved_sha` -- a content drift invalidates the prior reviewer approval (it was for the old requirements), so AUTO_MERGE must not land the PR until the reviewer re-evaluates against the updated body/comments. Both outcomes also share the same destination: a DIRECT bounce back to `validating`. Docs do not run here -- the single docs pass is deferred to the final-docs handoff after reviewer approval.
...
- **Internal flow**:
  ...
  8. Branch on the post-agent state. Every success exit calls one of two helpers (`_advance_after_docs_push` / `_advance_after_docs_no_change`) that route to **`in_review`** (clear the `final_docs_approval_seeded` sentinel; for the push branch update `agent_approved_sha` to the new head **only when `final_docs_approval_seeded` was set** — when validating's `gh.get_pr()` raised the seeding block never ran, so any non-`None` `agent_approved_sha` left in state is a stale snapshot from a prior round and must NOT be promoted to the new docs head, leaving AUTO_MERGE gated; and ratchet `pr_last_comment_id` via `_ratchet_in_review_watermark_for_final_docs` past any issue-thread reply the awaiting-human resume consumed so the next in_review tick does not bounce the issue to `fixing` over already-addressed feedback). Branches:
     ...
     - no commit + `DOCS: NO_CHANGE` verdict:
       - if `ahead > 0` (a prior tick committed but never landed the push), push the recovered commit and advance via `_advance_after_docs_push(after_sha)` — AUTO_MERGE must gate on the new head, so the local-only commit can't be left behind. A push failure parks with `push_failed`.
...
The "route to `fixing` on a new PR comment" arc is intentional: the fixing stage owns the dev-resume + push + hand-back-to-`validating` cycle so the in_review handler stays focused on PR-state terminals and the auto-merge gate. The dev resume and reviewer re-run still happen — they just live in different stages — so the "validating re-runs after a fix" guarantee holds. Docs do not run on the pushed-fix exit: the single docs pass is deferred to the final-docs handoff after reviewer approval.
   ...
   In_review terminals and fix bounce:
     ...
     in_review --(AUTO_MERGE on, approved, unmergeable past gates)──►
       resolving_conflict --(any pushed resolution: clean rebase,
         ...
         validating  (docs do not run here; the single docs pass
