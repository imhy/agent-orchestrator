...
- **Human merge by default.** The orchestrator is permanently manual-merge-only — `_handle_in_review` pings the HITL handles for a mergeable PR but never calls `gh.merge_pr`. A human clicks Merge on every PR that lands.
