...
| `DASHBOARD_PARALLEL_READS` | _(unset, off)_                   | opt-in switch for the Streamlit dashboard's parallel read fan-out. When set to `1` / `true` / `on` / `yes` (case-insensitive), `orchestrator/dashboard.py::main()` dispatches its 13 independent widget reads across a `ThreadPoolExecutor` (eight workers) so each worker pays one psycopg handshake against its own thread-local `analytics_connection` and the wall-clock collapses to roughly the slowest reader in each in-flight wave. Default off keeps the sequential path on the Streamlit render thread so operators can A/B against the sequential baseline by toggling the variable and reloading. Parsed at module import like `ANALYTICS_DB_URL`; the polling loop never reads it. Every dashboard load emits an INFO line of the form `dashboard.load: total=X.Xs reads=13 parallel=true\|false` so the two paths can be compared with a single grep against the dashboard log. |
...
5. **Launch the dashboard.** Install the optional `dashboard` dependency group once, then run Streamlit:
   ...
   Streamlit prints a `http://localhost:8501` URL; open it in a browser. The dashboard process is independent of the polling tick (no shared state, no GitHub access, read-only Postgres) and can be killed and relaunched without affecting workflow progress. Re-run step 4 to pick up new records — every widget (insight banners, KPI deltas, charts, expensive-issues table, drill-down) reads through a `st.cache_data` layer keyed by the active window, repo, events, stages, and issue filters, so changing any sidebar control invalidates the cache and re-queries Postgres without needing a Streamlit restart.
...
The sync accumulates validated rows into a `_BATCH_SIZE`-sized buffer (default 500) and flushes each batch with `cur.executemany("INSERT ... ON CONFLICT (content_hash) DO NOTHING", batch)`, so repeated runs are idempotent — even after `analytics.prune_old_records` rewrites the JSONL file and shifts source-line numbering — and a multi-thousand-record replay pays one Postgres round-trip per batch instead of one per row. Before the input file is opened the sync issues a single `SELECT content_hash FROM analytics_events WHERE content_hash IS NOT NULL` and snapshots the result into an in-process set so already-present rows are skipped in Python before they enter the batch; the same set absorbs intra-file duplicates as the loop iterates, so two identical records inside one JSONL file pay one round-trip not two. A 100 %-duplicate re-sync therefore issues one SELECT plus zero `executemany` calls; the `ON CONFLICT (content_hash) DO NOTHING` arbiter stays the authoritative dedup backstop for the rare case where a concurrent writer lands a hash between the pre-check and the flush. A final partial batch is flushed at EOF so the tail still lands. Per-batch `cursor.rowcount` drives the cumulative `inserted` / `skipped_duplicate` totals (duplicates = `len(batch) - rowcount` for wire-side skips plus the pre-skip counter for in-Python skips). Malformed lines (blank, non-JSON, non-object, or missing the required `ts` / `repo` / `issue` / `event` keys) are logged and counted but never enter the batch buffer, so a single bad line cannot abort the surrounding pipelined INSERT; the JSONL file is treated as read-only. A driver-level error inside a batch flush rolls the transaction back and propagates, so the CLI exits non-zero rather than reporting "success" on a half-inserted run. The sync is a no-op (no connection attempt) when `ANALYTICS_DB_URL` is unset or disabled, when `ANALYTICS_LOG_PATH` is explicitly disabled (`ANALYTICS_LOG_PATH` defaults to `LOG_DIR/analytics.jsonl` — only the empty value or `off` / `disabled` / `none` turns the sink off), or when the JSONL file is absent — so the CLI is safe to schedule before the Postgres service is deployed.

While the sync runs it surfaces operator feedback through the module logger and the stdout summary. Every log line is timestamped (UTC, with an explicit `UTC` suffix), a `connecting to <redacted-url>` / `connection established` pair brackets the connect call so a remote-Postgres reachability problem surfaces immediately, a `progress lines=N inserted=… duplicate=… malformed=… elapsed=…s` record drops after each batched `executemany` flush (full or final partial; the constants `_BATCH_SIZE` and `_PROGRESS_INTERVAL` in `orchestrator/analytics/sync.py` are both 500 by default so each flush carries one progress line at the operator's existing cadence), and a final `completed in %.3fs (…)` line carries the wall-clock total. The CLI prints a UTC-stamped one-line stdout summary at the end carrying `inserted=` / `duplicate=` / `malformed=` / `total_lines=` / `duration_s=`, so an operator piping stdout + stderr together gets a coherent time-ordered stream regardless of the host's local timezone and the elapsed cost of a multi-thousand-record replay is visible without grepping the log. `ANALYTICS_DB_URL` credentials are stripped before logging — both the `user:password@` netloc form and the libpq query-string form (`?user=`, `?password=`, `?sslpassword=`, `?passfile=`, case-insensitive per the libpq parameter-name rules) collapse to `***`, so credentials never land in the operator's terminal or log aggregator. See [`docs/observability.md`](observability.md#analytics-database-analytics-db) for the full operator-feedback reference.

### Streamlit dashboard

`orchestrator/dashboard.py` is the Streamlit app that visualizes the populated `analytics_events` table together with the `analytics_agent_runs` view and the `analytics_daily_rollup` materialised view defined alongside it. The read split is described in detail under [Analytics database in docs/observability.md](observability.md#analytics-database-analytics-db); briefly, the dashboard's window-bounded aggregates (summary KPIs, time series, stage / repo / backend breakdowns, throughput) read from the daily rollup so the previous events-table scans collapse to a day-keyed range scan, while the per-row drill-downs (recent agent exits, top-cost issues, the issue trace, hourly heatmap) and the widgets the rollup cannot reconstruct exactly (review-round breakdown, cost coverage) keep reading from the base table or the agent-run view. It is opt-in via a separate `dashboard` dependency group so the default `uv sync --locked` keeps installing only the polling runtime plus `pytest` / `ruff`. Streamlit (and its transitive pandas), `plotly`, the Plotly figure builders in `orchestrator/dashboard_charts.py`, and the plotly-free theme tokens in `orchestrator/dashboard_theme.py` are imported lazily inside `main()` -- so importing `orchestrator.dashboard` from a test or a non-dashboard caller does not require the group to be installed. `.streamlit/config.toml` carries the dashboard theme + `[browser] gatherUsageStats = false` opt-out so the chrome matches the figures and the launch stays local-observability-only. The lazy-import guard in `tests/test_dashboard.py` asserts that loading `orchestrator.dashboard` keeps `streamlit`, `pandas`, `plotly`, and `orchestrator.dashboard_charts` out of `sys.modules`.
...
The page chrome mirrors the standalone analytics mock (#341): a sticky topbar carries the page title with the data extent / repo / event summary on the left and the in-range spend pill on the right, and a sticky filter bar right below it exposes the inline `3D` / `7D` / `All` preset radio plus two date inputs for arbitrary windows within the data extent. The presets are anchored at the data extent's max timestamp (not today) and clamped to the data extent's min, so a freshly-deployed Postgres whose latest event is days old still surfaces a useful default window without the operator having to reach for a calendar. A `Custom` preset stays available in the sidebar as a fallback alongside the repo selector, multi-selects for events and stages, and a `#123` / `123` issue-number input. Every per-filter read is wrapped in `st.cache_data` keyed by `(start, end, repo, events, stages, issue)`, so a filter change invalidates every cached query in lockstep; `get_data_extent` and `get_filter_options` carry no filter inputs and live in argument-less wrappers under the longer `STATIC_METADATA_TTL_SECONDS = 300` (5 min) TTL so the sidebar / topbar only re-hit Postgres when `analytics.sync` ingests new events. The 13 widget reads are staged into two waves so the topbar / filter meta / insight banners / KPI strip paint as soon as their inputs are available rather than blocking on every widget; the second wave is skipped on an empty window. A single inline `st.spinner("Loading analytics…")` brackets both waves so the cold load shows immediate feedback. Clearing a multiselect means "show nothing for this dimension" (an empty selection emits a tautologically-false SQL predicate). The all-selected default is the unfiltered shape: for the event multiselect the dashboard passes the full set through (loss-free because `event` is `NOT NULL` in the schema), and for the stage multiselect it passes `None` instead — `options.stages` only enumerates the non-null stages the DB has seen, so emitting a `stage IN (...)` clause would silently drop legitimate NULL-stage rows (`stage_evaluation` writes a null stage on issues with no workflow label). Entering an issue number narrows every widget to that issue *and* renders a per-issue event drill-down — both behaviors require a specific repo, because GitHub issue numbers are not unique across repos; without one, the issue input stays inert and the drill-down section shows an instructive notice instead.

The body renders the redesigned layout, top to bottom: computed insight banners (failure rate ≥ 10 %, unpriced cost coverage ≥ 10 %); a four-tile KPI strip (total spend, total tokens, cost / resolved issue, rework share — each with an inline-SVG sparkline and a previous-window delta where applicable, and total tokens computed as `input + output + cache_read + cache_write` to mirror the mock's accounting); the hero `usage_over_time` stacked-area + cost-line chart with a "By token type / By backend" toggle (the token-type stack renders Input / Output / Cache bands, the backend stack reads `get_backend_daily_tokens` so it covers every agent run in the window); side-by-side `cost_by_stage` and `cost_by_review_round` horizontal-bar cards; a 7/5 split between the top-cost issues table (custom-HTML, sourced from `get_issues(sort_by="cost")` so the SQL itself ranks by `SUM(cost_usd) DESC` — columns: Issue with an in-row cost bar, Cost, Runs, Review rds, Retries, and a `clean` / `{n} fail` status pill) and the backend-efficiency cards (`$ / 1M tok`, `% cache hit`, `$ / run`) above the cost-source coverage bar (sized by token share); another 7/5 split between `cost_by_repo` bars and a six-tile reliability panel (agent runs / success rate / resolved / rejected / failures / timeouts — every numeric tile is sourced from the same `Summary` window-wide aggregate) above the issues-resolved-per-day bar chart with explicit zero days backfilled across the window; the 7 × 24 weekday × hour activity heatmap rendering token volume (matching the mock's "Token volume by hour × weekday" framing rather than raw event count); the recent agent-runs table as a collapsible expander; and the per-issue drill-down at the bottom.
...
**Parallel read fan-out.** Setting `DASHBOARD_PARALLEL_READS=on` (or `1` / `true` / `yes`, case-insensitive) flips `main()`'s 13 widget reads from the sequential loop on the Streamlit render thread to a `ThreadPoolExecutor` capped at eight workers. Each worker opens its own thread-local psycopg connection via `analytics.read.analytics_connection()` — `psycopg.Connection` is not thread-safe for concurrent use, so sharing one socket across workers would corrupt the wire protocol; the per-thread cache keeps each worker isolated. The default is off so the previous behavior holds until an operator opts in; flip it on once the sequential path has been measured against the parallel path in your deployment. The fan-out emits a single INFO log line on every dashboard load — `dashboard.load: total=X.Xs reads=13 parallel=true|false` on a full render, or `reads=6` when the empty-window short-circuit skips the second wave — so the two paths can be A/B'd with `grep dashboard.load streamlit.log`. An `AnalyticsReadError` raised by any worker propagates verbatim from the first failing future and surfaces as the same `st.error` the sequential path emits, so the parallel path's user-facing error story is identical to the sequential one.
...
| `No analytics events have been recorded yet. …` (top-level `st.info`, app stops)                  | data             | The data extent is empty — the `analytics_events` table holds zero rows. Confirm the JSONL sink is on (`ANALYTICS_LOG_PATH`), that recent workflow activity produced records, and run `python -m orchestrator.analytics.sync` to populate Postgres.                                                                  |
| `No analytics events match the current filters.` (page banner)                                    | data             | The data extent is non-empty but every row was filtered out. The empty-window guard short-circuits the body to this banner; widen the window preset, pick `All` for the repo, blank the issue-number input, and confirm the event / stage multi-selects still have **every option selected** (an empty multi-select is the documented "show nothing" signal — see the note below the table).                                                                                                          |
| `No stage data matches the current filters.` (chart annotation)                                   | data             | Scoped to the stage breakdown chart. The widget is also empty when the only matching rows have a NULL stage (`stage_evaluation` records on issues with no workflow label).                                                                                                                                          |
| `` No `agent_exit` rows match the current filters. ``                                            | data             | The window contains `stage_enter` / `stage_evaluation` rows but no agent invocations — surfaces in the review-round chart, backend cards, cost coverage bar, and recent-runs expander. Common right after starting up or while issues sit on non-agent stages. Run a fresh sync if you expect newer rows.            |
...
| `DASHBOARD_PARALLEL_READS` | next `streamlit run orchestrator/dashboard.py` start. Parsed at dashboard import; the polling loop never reads it, so the only restart needed is the Streamlit process |
