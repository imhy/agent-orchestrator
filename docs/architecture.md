...
orchestrator/
  ...
  verify.py             — local-verify runner and worktree-state probes:
                           ...
                           `_worktree_dirty_files`. Used by
                           `_handle_validating`'s pre-handoff gate (before
                           the final-docs flip to `documenting` and the
                           eventual `in_review`) and by the dirty / HEAD
                           probes the stage handlers run after every
                           agent step. Every name here is re-exported
  ...
  branch_publication.py — PR branch publication helpers extracted from
                           ...
                           `_squash_and_force_push`. Imports the
                           hardened git plumbing from
                           `git_plumbing.py` and the worktree-state
                           probes from `verify.py`; every name here
                           is re-exported from `worktrees.py` so
                           existing imports and `patch.object(worktrees,
                           "_foo", ...)` test patches keep working.
...
| `_handle_*` per issue | function call | issue's workflow label | once per tick per open issue (within its repo's `tick`); concurrent up to `spec.parallel_limit` per repo and `MAX_PARALLEL_ISSUES_GLOBAL` across all repos (single shared `IssueScheduler`). No-agent family buckets (`blocked` / `umbrella` only) are submitted cap-exempt so a cheap-polling parent cannot be starved by ordinary implementation work |
...
| **branch_publication.py** | PR branch publication helpers (`_CONVENTIONAL_RE`, `_is_conventional_subject`, `_first_commit_subject`, `_pr_title_from_commit_or_issue`, `_branch_ahead_behind`, `_squash_and_force_push`); all re-exported from `worktrees.py` |
...
| **worktrees.py** | compatibility re-export hub for `git_plumbing.py`, `verify.py`, `worktree_lifecycle.py`, `branch_publication.py`, and `base_sync.py` |
