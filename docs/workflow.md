...
The stage handlers themselves live under `orchestrator/stages/` after the workflow split — `decomposition.py` owns the decomposing / ready / blocked / umbrella handlers, `implementing.py` owns the dev-session lifecycle (and relabels straight to `validating` after the PR opens; docs run once after reviewer approval, not here), `documenting.py` owns the **single docs pass** between reviewer approval and `in_review` (entered only via the `documenting` label set by `_handle_validating`'s approval branch). Success exits always advance to `in_review` and a pushed docs commit also updates `agent_approved_sha` to the new head, gated on the sentinel `final_docs_approval_seeded` that validating sets only when it actually persisted a non-empty `agent_approved_sha` this round — both `gh.get_pr()` succeeded AND `_head_sha()` returned a non-empty local SHA. When either fails the sentinel is absent, and any stale `agent_approved_sha` left over from a prior round will NOT be promoted to the docs head. The final-docs exit also ratchets `pr_last_comment_id` past any issue-thread reply that the awaiting-human resume consumed, so the next in_review tick does not replay it as fresh PR feedback. A user-content drift mid-hop invalidates the prior approval: the handler clears `agent_approved_sha`, resets `review_round=0`, and relabels back to `validating` without spawning the docs agent; before the relabel it fetches `<remote>/<branch>`, probes HEAD inline (so a probe failure is distinguishable from a real "in sync" result), and -- when the local branch is ahead of remote, behind remote (the remote moved past local while documenting was in flight), OR the worktree is dirty -- runs `git reset --hard <remote>/<branch>` + `git clean -fd` so the next reviewer round runs against the actual remote PR head and no unpushed local docs commit / uncommitted / untracked docs edits authored against the OLD body survive. The approval bookkeeping clear runs before any fallible step, so each park (`fetch_failed` on fetch failure, `worktree_reset_failed` on probe / reset / clean failure) leaves no stale `final_docs_approval_seeded` sentinel an operator unpark could ride into a fresh final-docs handoff. The drift block also persists `docs_drift_unwind_pending=True` while a cleanup is in progress and clears it only on the success path that relabels to `validating`; an operator unpark or fresh human comment re-enters the drift block on the next documenting tick to retry the cleanup, so an unpark cannot fall through to a docs spawn or recovered-commit shortcut. `validating.py` owns the reviewer-session lifecycle (any pushed dev fix from CHANGES_REQUESTED, awaiting-human resume, user-content drift, or transient-park recovery stays on `validating` and clears `agent_approved_sha`; the reviewer re-evaluates against the new head on the next tick. Approval + verify + squash seeds `final_docs_approval_seeded` alongside `agent_approved_sha` and relabels to `documenting` for the final-docs hop). `in_review.py` is permanently manual-merge-only: it owns the PR-watermark primitives, routes fresh PR feedback to `fixing`, pings HITL once per head SHA for a mergeable PR, parks awaiting human attention for an unmergeable PR, and never calls `gh.merge_pr`. Its own user-content drift bounces DIRECTLY back to `validating` on both the pushed-fix and no-commit ACK outcomes — docs do not run on the drift exit, the single docs pass runs only after reviewer approval. `fixing.py` owns the PR-feedback quiet-window + dev-resume + push + hand-back-to-`validating` cycle (both the pushed-fix exit and the no-new-feedback bounce flip directly to `validating`; docs do not run on the pushed-fix exit), `conflicts.py` owns `_handle_resolving_conflict` (every exit — pushed resolution (clean rebase, recovered push, agent-resolved conflicts, awaiting-human resume push, drift-pushed fix) or base-up-to-date no-op — hands straight back to `validating`; the single docs pass is deferred to the post-approval handoff to `documenting` in `_handle_validating`), and `question.py` owns `_handle_question`. The `_handle_pickup` entry handler (no label → decomposing / implementing) and the `_process_issue` label dispatcher still live in the facade module `orchestrator/workflow.py`, which re-exports every handler under its original `_handle_*` name; tests and intra-handler calls keep using the `workflow._handle_*` surface unchanged. `_comment_created_at` is also re-exported by the facade so the fixing handler can reach it through `workflow.<name>` for the quiet-window debounce (and test patches against `workflow.<name>` continue to intercept calls from inside the stage module). See [`architecture.md`](architecture.md#top-level-layout) for the full module map.
