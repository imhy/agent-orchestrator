...
| Implementer / dev| `DEV_AGENT`        | `claude` | `_handle_implementing`, `_handle_validating` (fix loop and awaiting_human resume), `_handle_fixing` (PR-comment resume, real handler still pending under parent #137 — `_handle_in_review` routes fresh PR feedback here instead of spawning the dev itself), `_handle_resolving_conflict` (conflict resume and awaiting_human resume) | Locked per issue after first spawn |
