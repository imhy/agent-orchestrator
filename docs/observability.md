...
4. Side-by-side `cost_by_stage` and `cost_by_review_round` cards; the review-round card groups development and review cost bars per round, and each role's bar is further stacked into no-cache + cache cost so the operator can see how much per-round spend still bypasses prompt caching.
