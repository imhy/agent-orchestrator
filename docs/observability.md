...
| `conflict_round` | `_route_pr_worktree_to_resolving_conflict` emits `action="entered"`; every increment site (`_emit_conflict_round_incremented`) emits `action="incremented"` with `outcome` | `pr_number`, `conflict_round`, `review_round`, `retry_count`, `outcome` (for increments), `sha` |
...
| `get_hourly_heatmap` | base table | 7×24 weekday/hour activity cells from `EXTRACT(DOW)` / `EXTRACT(HOUR)` with per-cell event count + `input + output + cache_read + cache_write` token total. |
...
4. Side-by-side `cost_by_stage` and `cost_by_review_round` horizontal-bar cards.
...
7. 7 × 24 weekday × hour activity heatmap rendering token volume.
8. Recent agent-runs table as a collapsible expander.
