...
**Skill-trigger follow-ups (not implemented).** Two further surfaces are intentionally out of scope for this first cut and live only as follow-ups in [`../plans/skill-trigger-tracking.md`](../plans/skill-trigger-tracking.md): a per-invocation `skill_triggered` audit event on [`EVENT_LOG_PATH`](#audit-event-log-event_log_path) (the `agent_exit` rollup already answers the headline questions, and one record per run avoids an event-count explosion on multi-skill runs), and a skill-trigger-rate dashboard widget over the new fields (a pure read-side addition once data accumulates, since `extras JSONB` already stores them with no schema change). Neither ships today — the audit [event-kinds table](#audit-event-log-event_log_path) does not list `skill_triggered`, and the dashboard exposes no skill widget.
