...
**Skill-trigger extractor (opt-in).** A sibling trio mirrors the usage parsers' two-parsers-one-dispatcher shape and resilience contract to record which agent *skills* a run triggered, gated behind `TRACK_SKILL_TRIGGERS` (default off; see [`agent_exit` records](#agent_exit-records)). `parse_claude_skills(stdout)` reads the firm signal — `tool_use` content blocks named `Skill` in the `assistant` stream — and returns `input.skill` in first-seen order. `parse_codex_skills(stdout)` is **best-effort**: the precise `codex exec --json` skill-event shape is an open capture task, so it scans for a `Skill`-named tool/function call or a `*skill*`-typed event and may return empty until a real codex stream pins the shape — codex is covered rather than scoped out because the default reviewer (`REVIEW_AGENT=codex`) is the most common skill-trigger case. `parse_agent_skills(backend, stdout)` dispatches by backend exactly as `parse_agent_usage` does. The offered-skills set (`SkillTriggers.available`) is likewise **best-effort** and stays empty until a captured `claude` / `codex` stream confirms its field; the *triggered* set does not depend on it. As with the usage parsers, malformed JSONL lines are skipped and a missing / renamed field yields an empty result rather than an exception. Only the skill *name* is ever read — never the `Skill` tool's `args` (Privacy).
