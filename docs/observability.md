...
| `get_review_round_breakdown` | agent-run view | per `review_round_bucket` runs / failed counts + `total_cost_usd`, plus per-role (`developer_*` / `reviewer_*`) run counts and cost, each role's cost further split into cache vs no-cache (`*_cache_cost_usd` + `*_no_cache_cost_usd`; a run counts as cache-using when it billed any `cached_tokens` / `cache_read_tokens` / `cache_write_tokens`). NULL buckets surface as `"unknown"`. |
