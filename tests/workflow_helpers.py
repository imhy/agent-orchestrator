...
class _PatchedWorkflowMixin:
    ...
    def _run(
        ...
        *,
        ...
        # The analytics module's default points at
        # `<LOG_DIR>/analytics.jsonl` under the repo root, so leaving it
        # untouched lets every workflow test that drives a stage handler
        # scribble into the operator's real log directory. Default the
        # patch to None (sink disabled, the documented "off" knob) so
