...
| Sync (`INSERT … ON CONFLICT (content_hash) DO NOTHING`, malformed-line tolerance, driver-error rollback, no-op when disabled / file missing) | `tests/test_analytics_sync.py` |
| Read model (`get_filter_options`, `get_data_extent`, `get_summary`, `get_time_series`, `get_stage_breakdown`, `get_event_breakdown`, `get_recent_agent_exits`, `get_issues`, `get_issue_events`, `get_repo_breakdown`, `get_hourly_heatmap`, `get_throughput_breakdown`, `get_review_round_breakdown`, `get_backend_daily_tokens`, `get_backend_efficiency`, `get_cost_coverage`) | `tests/test_analytics_read.py` |
