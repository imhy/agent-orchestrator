...
6. **File a bootstrap test issue** to verify the path works end-to-end:
   ...
   The orchestrator then pushes the branch, opens a PR, labels the issue `validating`, runs a fresh reviewer session against the diff, and on `VERDICT: APPROVED` runs the optional local `VERIFY_COMMANDS` (default empty) in the worktree before moving the issue to `in_review`. A verify failure parks on `validating` with a typed `park_reason` and the failing command's output; GitHub CI remains the later auto-merge gate. See [`docs/configuration.md#local-verification-gate`](docs/configuration.md#local-verification-gate) for the verify-command reference.
